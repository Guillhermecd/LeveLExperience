...
Gerado em 2026-09-15T13:51:38.808Z

**Situação: APROVADO** — 106/106 aprovados
...
| | Cenário | ms |
|---|---|---|
| ✅ | never sends an Idempotency-Key on a GET request | 75 |
| ✅ | sends a freshly generated Idempotency-Key on a mutating request | 4 |
| ✅ | retries after a 401 with the SAME Idempotency-Key it used on the first attempt | 5 |
| ✅ | does not retry a 401 on auth endpoints (skipAuthRetry) | 3 |
| ✅ | throws ApiError with the parsed message on a non-2xx response | 4 |
| ✅ | tolerates a non-JSON error body (e.g. a missing-header 400 from Spring defaults) | 4 |
...
| | Cenário | ms |
|---|---|---|
| ✅ | the table is not empty and has no duplicate ids | 4 |
| ✅ | card-done-low | 3 |
| ✅ | card-done-medium | 1 |
| ✅ | card-done-high | 0 |
| ✅ | card-undone | 0 |
...
| ✅ | no event has a zero delta | 7 |
| ✅ | the raw total is always the ledger sum plus the opening balance | 1 |
| ✅ | applying zero actions never emits an event | 3 |
