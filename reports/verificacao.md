...
Gerado em 2026-09-15T17:00:52.071Z
...
**132/132 aprovados** · 0.36s

### apiRequest — 6/6

| | Cenário | ms |
|---|---|---|
| ✅ | never sends an Idempotency-Key on a GET request | 51 |
| ✅ | sends a freshly generated Idempotency-Key on a mutating request | 3 |
| ✅ | retries after a 401 with the SAME Idempotency-Key it used on the first attempt | 9 |
| ✅ | does not retry a 401 on auth endpoints (skipAuthRetry) | 4 |
| ✅ | throws ApiError with the parsed message on a non-2xx response | 3 |
| ✅ | tolerates a non-JSON error body (e.g. a missing-header 400 from Spring defaults) | 2 |

### boardCache — 4/4

| | Cenário | ms |
|---|---|---|
| ✅ | returns null when nothing was ever cached | 4 |
...
| | Cenário | ms |
|---|---|---|
| ✅ | lists a month with the range as query params and no Idempotency-Key | 69 |
| ✅ | creates with POST, the client-generated id in the body and an Idempotency-Key | 11 |
| ✅ | updates with PATCH on the event path and an Idempotency-Key | 2 |
| ✅ | deletes with DELETE and tolerates the empty 204 body | 2 |
| ✅ | returns the parsed events on a successful list | 10 |
| ✅ | resolves to an empty list when the month has no events | 5 |
| ✅ | rejects with the server message when the event belongs to someone else (404) | 8 |
| ✅ | rejects with an ApiError on a delete that hits 404 | 2 |
| ✅ | rejects with a fallback message when the error body is not the ErrorResponse shape | 3 |
| ✅ | rejects with a validation message when the server refuses the payload (400) | 2 |
| ✅ | propagates a network failure instead of swallowing it | 2 |
| ✅ | sends nulls for an event with no description and no declared end | 1 |
| ✅ | keeps unicode in the title and description intact through the json body | 1 |
...
| | Cenário | ms |
|---|---|---|
| ✅ | includes an event starting within the next 2 hours | 6 |
| ✅ | excludes an event more than 2 hours away | 1 |
| ✅ | excludes an event that already started | 1 |
| ✅ | includes an event starting at exactly now | 1 |
| ✅ | excludes an event starting exactly at the end of the 2h window | 0 |
| ✅ | includes an event one minute inside the window edge | 1 |
| ✅ | returns an empty list when there are no events at all | 0 |
| ✅ | excludes an event already in progress, even if it ends inside the window | 1 |
| ✅ | keeps every event inside the window, in the order received | 1 |
...
| ✅ | excludes a goal that is already done, even near the deadline | 1 |
| ✅ | changes the alert id across periods so a dismissed overdue goal alerts again next period | 1 |
| ✅ | changes the alert id across months for a month goal | 1 |
...
| ✅ | returns an empty list when there are no goals at all | 3 |
...
| | Cenário | ms |
|---|---|---|
| ✅ | names the weekday in Portuguese for a week goal | 1 |
...
| ✅ | round-trips a dismissed set | 1 |
| ✅ | round-trips an empty set as an empty set, never as null | 0 |
| ✅ | returns an empty set for corrupt json instead of throwing | 1 |
| ✅ | returns an empty set when the stored value is valid json but not an array | 1 |
| ✅ | drops non-string entries from a mixed array | 0 |
| ✅ | returns an empty set when localStorage itself throws on read | 1 |
| ✅ | swallows a write failure (quota) instead of breaking the dismiss | 2 |
...
| | Cenário | ms |
|---|---|---|
| ✅ | spans the first to the last instant of the local month | 9 |
| ✅ | rolls over the year boundary (December to January) | 2 |
| ✅ | accepts a native Date as well as a Dayjs | 1 |

### groupByDay — 7/7

| | Cenário | ms |
|---|---|---|
| ✅ | returns days ascending and events sorted by time inside a day | 33 |
| ✅ | keeps an event that crosses midnight in the day it starts | 4 |
| ✅ | returns an empty list for an empty month | 0 |
| ✅ | does not mutate the array it was given | 1 |
| ✅ | keeps two events that start at the same instant, in the order received | 1 |
| ✅ | splits the first and the last instant of a day into their own days | 1 |
| ✅ | groups a whole busy month without losing an event | 27 |

### dayKeyOf — 2/2

| | Cenário | ms |
|---|---|---|
| ✅ | uses the local day of the instant, which is what the month grid renders | 3 |
| ✅ | rolls to the next local day right after local midnight | 1 |

### eventColorOptions — 3/3

| | Cenário | ms |
|---|---|---|
| ✅ | offers exactly one option per color in the union | 6 |
| ✅ | labels every option in Portuguese and non-empty | 1 |
| ✅ | takes every dot straight from the design tokens, never a literal hue | 4 |
